...
''' 
        ...
        All the following views serve the latest version of the project
...
def index3theme3(request):
    ...
    if request_code == 0:
        ...
        return render(request,"REQUEST TYPE ERROR")
#following views render various html contents as required by ajax calls 
def bootstrapgridbtheme3(request):
    return render(request,"adminpanel/bootstrap-grid.html")
...
def course_enrollment_detail_on_a_date(request):
    ...
    datalist=[]
    datalist=get_course_enrollment_detail_on_a_date(ip=MYSQL_SERVER_IP,user=USER,paswd=PASSWORD,db=DATABASE,page_no=page_no,records_per_page=records_per_page,page_no1=page_no,records_per_page1=records_per_page,date=date1)
    return render(request,"adminpanel/course_enrollment_detail_on_a_date.html",{'data':datalist[0],'data1':datalist[4]})
...
def email_details(request):
    ...
    return render_to_response('adminpanel/email_details.html',{'data':datalist[0],'page_no':datalist[1],'records_per_page':datalist[2],'max_page':datalist[3],'startdate':sd,'enddate':ed})
