...
def getall_course_grades(user_id):
	...
	#print "Executing query"
        strquery2="Select grade,max_grade from courseware_studentmodule where max_grade is not null and grade is not null and student_id=%s and module_id in (%s)"    
        strquery="select user_id,course_id from student_courseenrollment where is_active=1 and course_id=%s"
        query="select course_id,count(user_id) as no_of_students from student_courseenrollment where course_id IN (select course_id from student_courseaccessrole where user_id=\'"+user_id+"\' and role='instructor' ) group by course_id"
...
def getall_student_grades(course_name):
        ...
        query="select a.id, a.username, a.email, b.course_id from auth_user as a, student_courseenrollment as b where a.is_active=1 and a.id=b.user_id and b.course_id like '%"+course_name+"%'"
	#print strquery
	...
	y=mysql_cur2.fetchall()
	#print y
	x=get_problem_ids2(db_mysql,client,course_name)
	#print x
	if x==-1:
		return ([],[])
	else:
	...
	for k_format in format_newlist:					#Query to get the grades for the student for that particular problem
                ...
                for k in sequential_problem_list:
					...
					avg_score_sequential=sum_prob_score_obt/sum_tot_prob_score		#Calculating avg score of this sequential
                                ...
                                if count==1:
                                        ...
                                        highchart_list2.append(str(round(avg_score_sequential,2)))
                                else:
                                        highchart_list2.append(str(round(avg_score_sequential,2)))
				best_score_list.append(avg_score_sequential)		#Adding the sequential score to best_score_list
		best_score_list.sort(reverse=True)  #Sorting the scores list for that format in descending order
		...
		if sequential_coun-drop_count>0:
                        ...
                        if sequential_coun-drop_count>1:
                                if count==1:
                                        ...
                                        highchart_list2.append(str(round(avg_score_format,2)))
                                else:
                                        highchart_list2.append(str(round(avg_score_format,2)))
                        stud_avg_tot+=avg_score_format*weight
                else:
        ...
        if len(highchart_list2)>0:
                if count==1:
                        ...
                        highchart_list2.append(str(round(stud_avg_tot,2)))
                else:
                        highchart_list2.append(str(round(stud_avg_tot,2)))
	#highchart_list2=[]
