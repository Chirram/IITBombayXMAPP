...
def emais_count_coursewise(ip,user,pwd,db,page_no,no_of_entries,date1,date2):
    #print "hellooooo"
     #!/usr/bin/python
    ...
    try:
        ...
        #print "select count(distinct course_id) as count,count(*) as total_emails from bulk_email_courseemail where date(created)>=\'"+date1+"\' and date(created)<=\'"+date2+"\';"
        ...
        cursor.execute("SELECT date(created) as dt,course_id, count(*) as total,subject from bulk_email_courseemail where date(created)>=\'"+date1+"\' and date(created)<=\'"+date2+"\'"+" group by course_id,date(created) order by date(created) limit "+str(no_of_entries)+" offset "+str(offset)+" ;")
        data=cursor.fetchall()
        results=[]					#a list to store all data			
        results.append(data)
        results.append(page_no)
        results.append(no_of_entries)
        results.append(int(total_pages))
	#print "-------------------->total emails  ::  ",count[0][1]
        results.append(count[0][1]) #this is appending the total number of pages required.
    except: 
