...
def students_of_course_result_display(request,faculty_id,orgname,coursename,semester,page_no,no_of_entries,search):
	...
	try:
		...
		if(count==0): # user is not authorized to access the course
			...
			try:
				...
				return render_to_response('faculty/students_of_course_result_display.html', {'data':data, 'cname':cname,'total_records':total_records, 'crslist': crslist,'fac_id':faculty_id,'page_no':page_no, 'total_pages': total_pages, 'no_of_entries':no_of_entries,"search":search,'uname': str(request.session["username"])})
			except:
...
def course_unanswered_questions(request,facultyid) :
        ...
        if request.method == 'POST':
        	...
        	print "post"+course       	
      		...
      		course = ""
	return render_to_response('faculty/course_unanswered_questions_report.html',{'data':get_unanswered_questions_report(facultyid,course) ,'course_list' : course_list, 'fid' : facultyid, 'courseid' : json.dumps(course) ,'uname': str(request.session["username"])})
...
def course_discussions(request,facultyid) :
        ...
        if request.method == 'POST':
        	...
        	print "post"+course       	
      		...
      		course = ""
      	print course +"123"
	return render_to_response('faculty/course_discussions_report.html',{'data':get_discussions_report(facultyid,course) ,'course_list' : course_list, 'fid' : facultyid,'courseid' : json.dumps(course),'uname': str(request.session["username"])})
...
def course_answered_questions(request,facultyid) :
        ...
        if request.method == 'POST':
        	...
        	print "post"+course       	
      		...
      		course = ""
      	print course +"123"
	return render_to_response('faculty/course_answered_questions_report.html',{'data':get_answered_questions_report(facultyid,course) ,'course_list' : course_list, 'fid' : facultyid,'courseid' : json.dumps(course),'uname': str(request.session["username"])})



#-----------------------------------------------------------------------------------------------------------------
# PARENT METHOD FOR SECOND LEVEL GRADE REPORT
def students_grade_courselevel(request,courseid,facultyid) :
        (newlist1,newlist2)=getall_student_grades(courseid)	
	return render_to_response('faculty/courselevel_grades.html',{'data':newlist1,'data1':newlist2 ,'courseid' : courseid ,'fid' : facultyid ,'uname': str(request.session["username"])})

#-----------------------------------------------------------------------------------------------------------------
# PARENT METHOD FOR THIRD LEVEL GRADE REPORT
def students_grade_quizlevel(request,student_id,courseid,facultyid) :
        
	return render_to_response('faculty/quizlevel_grades.html',{'data':get_student_course_grades(student_id,courseid) ,'fid' : facultyid ,'uname': str(request.session["username"])})
