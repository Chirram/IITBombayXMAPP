...
def get_answered_questions_report(facultyid,courseid) :
	...
	try:    
		...
		discussion_db = client.cs_comments_service_development
		discussion_coll = discussion_db.contents
		ans_ques = discussion_coll.find({"_type" : "CommentThread" , "thread_type" : "question" ,"course_id" : courseid, "comment_count" : {"$gt" : 0 }} ,  {"_id" : 1,"body" : 1,"title" : 1,"commentable_id" : 1})
		for i in ans_ques:
			result.append([i['title'],i['body'],str(i['commentable_id']),str(i['_id'])])
		client.close()
		print result
		return result
	except:
